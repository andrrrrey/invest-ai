...
def extract_requested_amount(text: str, budget_summary: str = "") -> str:
    ...
    prompt = (
        ...
        "1. Если есть структурированный бюджет («ИТОГО за период» и «Среднемесячно») — используй «Среднемесячно» как основную сумму.\n"
        "2. Ищи в тексте явные маркеры итога: «Требуемый план ФОТ», «Итого», «Запрашивается», «Необходимо», «Общая сумма».\n"
        "3. Если в тексте есть «текущий лимит» и «требуемый план» — указывай ТРЕБУЕМЫЙ ПЛАН, а НЕ дельту/увеличение.\n"
        "4. Если есть только «увеличение на X» без итоговой суммы — укажи X.\n"
        "5. Если сумма не определена — верни «не указана».\n\n"
        "Верни ТОЛЬКО сумму с валютой и периодом (например: «696 047 ₽/мес» или «2 400 000 ₽/год»).\n"
