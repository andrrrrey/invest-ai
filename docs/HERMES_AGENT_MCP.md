...
| `search_knowledge` | `query: str`, `top_k=5` | Знания компании (глоссарий, регламенты, методология, FAQ): семантический поиск по эмбеддингам с фолбэком на keyword. Доступен при `knowledge_enabled` |
...
| `knowledge_enabled` | on | `HERMES_KNOWLEDGE_ENABLED` | База знаний: закреплённые знания в промпте + инструмент `search_knowledge` |

Дополнительно: настройка `embedding_model` (по умолчанию `text-embedding-3-small`)
— модель эмбеддингов для семантического поиска по базе знаний. Работает через
активного провайдера openai/routerai; при недоступности эмбеддингов поиск
деградирует до keyword (SQL ILIKE). Новое действие аудита `ai.embed` фиксирует
егресс на эмбеддинги, `knowledge.create/update/delete/reindex` — изменения базы.
